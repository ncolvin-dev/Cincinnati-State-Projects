...
IF OBJECT_ID ('TProductCategories')	IS NOT NULL DROP TABLE TProductCategories
...
-- 1
SELECT strFirstName, strLastName, dtmOrderDate, strProductName
FROM TCustomers
		JOIN TOrders
			ON TOrders.intCustomerID = TCustomers.intCustomerID
		JOIN TOrderProducts
			ON TOrderProducts.intOrderID = TOrders.intOrderID
		JOIN TProducts
			ON TOrderProducts.intProductID = TProducts.intProductID
ORDER BY TCustomers.intCustomerID, TOrders.dtmOrderDate 

-- 2
SELECT strProductName, strVendorName, strProductCategory, monRetailCost
FROM TProducts 
	 JOIN TVendors
		 ON TProducts.intVendorID = TVendors.intVendorID
	 JOIN TProductCategories
		 ON TProducts.intProductCategoryID = TProductCategories.intProductCategoryID
ORDER BY  monRetailCost DESC, strVendorName, strProductCategory

-- 3

SELECT strProductName, intInventory, strVendorName, strContactFirstName, strContactLastName, strContactEmail, strContactPhone
FROM TProducts 
	 JOIN TVendors
		 ON TProducts.intVendorID = TVendors.intVendorID
WHERE intInventory < 10

-- 4

SELECT strFirstName, strLastName, strRace, strProductName
FROM TCustomers
	    JOIN TRaces
			ON TCustomers.intRaceID = TRaces.intRaceID
		JOIN TOrders
			ON TOrders.intCustomerID = TCustomers.intCustomerID
		JOIN TOrderProducts
			ON TOrderProducts.intOrderID = TOrders.intOrderID
		JOIN TProducts
			ON TOrderProducts.intProductID = TProducts.intProductID
ORDER BY TRaces.strRace

-- 5

SELECT strVendorName, strProductName, strState
FROM TCustomers
		JOIN TOrders
			ON TOrders.intCustomerID = TCustomers.intCustomerID
		JOIN TOrderProducts
